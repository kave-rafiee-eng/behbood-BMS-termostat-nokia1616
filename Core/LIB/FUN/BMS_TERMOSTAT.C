...
#define lcd_temp_x 70
#define lcd_temp_y 92

#define lcd_Humidity_x 70
#define lcd_Humidity_y 42
		...
		if( bms.dark_mode == 0 ){
				if( tbr_g1[def_tbr_g1_read_touch].F_end ){ tbr_g1[def_tbr_g1_read_touch].F_end=0;
						...
						if( touch.seting > 0 ){
							...
							else tbr_g1[def_tbr_g1_seting_temp_hum].C_set_time=500;
...
void dark_mode_manage(){
		...
		if( bms.dark_mode == 1 ){
				...
				tbrc_s1[def_tbrc_s1_dark_mode].EN=0;
...
void BMS_TERMOSTAT_INI(){
	...
	tbrc_s1[def_tbrc_s1_dark_mode].C_set_time=10;
	tbrc_s1[def_tbrc_s1_dark_mode].EN=1;
